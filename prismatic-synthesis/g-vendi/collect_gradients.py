...
import dllm.utils as dllm_utils
...
def parse_args():
    ...
    parser.add_argument("--device_idx", type=int, default=None,
                        help="Data shard index (0..device_split_size-1). "
                             "Defaults to CUDA_VISIBLE_DEVICES if not set.")
...
def find_start_and_end_idx(args: Namespace) -> Tuple:
    cuda_visible_device = args.device_idx if args.device_idx is not None else int(os.getenv("CUDA_VISIBLE_DEVICES"))
...
def get_model_and_tokenizer(model_name_or_path: str):
    resolved_path = dllm_utils.resolve_with_base_env(model_name_or_path, "BASE_MODELS_DIR")
    model = AutoModelForCausalLM.from_pretrained(resolved_path, dtype="auto").cuda()
